...
public class AESDecrypt {
    ...
    // Inverse Mix columns - need to do the inverse? 
    ...
    private void mixColumns(int[][] state){
        for(int column = 0; column < 4; column++){
            ...
            state[0][column] = (mul(2, (byte)temp[0]) ^ temp[2] ^ temp[3] ^ mul(3, (byte)temp[1])) & 0xFF;
            state[1][column] = (mul(2, (byte)temp[1]) ^ temp[3] ^ temp[0] ^ mul(3, (byte)temp[2])) & 0xFF;
            state[2][column] = (mul(2, (byte)temp[2]) ^ temp[0] ^ temp[1] ^ mul(3, (byte)temp[3])) & 0xFF;
            state[3][column] = (mul(2, (byte)temp[3]) ^ temp[1] ^ temp[2] ^ mul(3, (byte)temp[0])) & 0xFF;
        }
    }
